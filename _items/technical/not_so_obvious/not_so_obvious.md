...
date: 2025-02-13 04:39:54 +00:00
layout: article
permalink: /technical/not_so_obvious/
description: ""
categories: [technical, white-paper]
tags: [technical, white-paper, assembly, assembly-instructions, x86, linker, gcc, ld.so]
author: "notweerdmonk"
draft: false
canonical_url: "https://notweerdmonk.github.io/technical/not_so_obvious/"
---
# Not so obvious

The [previous](/technical/tiny_elf/) post about reducing the size of an
executable that prints a short string to the standard output. In this post we
shall further reduce its size but only by a tinier bit.
...
There is not much scope for optimizations in the source code here. We need to
think in terms of functionality of the program, that is printing a string to the
standard output. If we constrain our assumptions of what printing to the
...
Both the file descriptors `0` and `1` for a shell represent a single file
`/dev/pts/2`. This is a character device. Notice something interesting?
...
The file is marked read-write for its owner. And writing to it indeed writes to
the terminal. Consequently writing to the standard input of the shell also
writes to the terminal!

---

In the assembly program, we can actually write to file descriptor `0` and still
print the string to the terminal. How can we leverage this? We shall pass 0 as
the second argument to the `write` syscall following its syscall number and this
